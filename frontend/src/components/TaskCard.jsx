...
const TaskCard = ({ task, onEdit, onDelete, userMap }) => {
    ...
    const borderColor =
        urgency === "overdue" ? "red" :
            urgency === "upcoming" ? "orange" :
                "transparent";

    const dateColor =
        urgency === "overdue" ? "error.main" :
            urgency === "upcoming" ? "warning.main" :
                "text.secondary";

    return (
        <Card sx={{ border: `2px solid ${borderColor}`, borderRadius: 2 }}>
            <CardContent>
                <Box display="flex" justifyContent="space-between">
                    <Box>
                        <Typography variant="h6">{task.title}</Typography>
                        <Typography color="text.secondary" sx={{ mb: 1 }}>{task.description}</Typography>
